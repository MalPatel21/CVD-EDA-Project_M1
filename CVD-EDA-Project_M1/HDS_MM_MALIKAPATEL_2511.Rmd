...
Cardiovascular disease (CVD) continues to be the leading cause of death globally, encompassing conditions such as coronary heart disease, stroke, and heart failure. Multiple risk factors for CVD—such as age, cholesterol, glucose, blood pressure, and gender—have been established; however, less is known about their combined effects and the role of pulse pressure, an indicator of arterial stiffness. This study used an open-source dataset consisting of 70,000 adult patients and 13 features based on an individual's demographics, behavior, and medical information. It aimed to evaluate the influence of age, gender, cholesterol level, glucose level, and pulse pressure on CVD risk.

Data preprocessing involved removing implausible values, converting numeric codes to categorical values, and deriving pulse pressure from systolic and diastolic blood pressures. At an individual level, exploratory visualizations showed trends in the occurrence of CVD across predictor categories. A multivariable logistic regression assessed the associations of these variables with CVD. Age, elevated pulse pressure, and high cholesterol were observed as significant positive predictors, with Well Above Normal cholesterol showing the strongest association. Glucose levels showed mixed results, with above-normal levels increasing risk and Well Above Normal levels associated with reduced risk. Gender was found to be statistically insignificant.

A receiver operating characteristic curve was used to evaluate model performance, showing adequate discriminatory ability (AUC = 0.75). These findings underscore the combined effects of risk factors, particularly the significant roles of increasing age, elevated pulse pressure, and high cholesterol in CVD risk. Overall, this study supports the complex nature of CVD and demonstrates how clinical and demographic data can be used to identify individuals at higher risk, allowing for earlier warning and intervention.


# Introduction
The term cardiovascular disease (CVD) is used to describe the wide array of diseases that impact the heart and blood vessels [@gaidai2023global]. Cardiovascular disease is the leading cause of death globally, causing 19.8 million deaths in 2022 [@who2025cvd]. CVD encompasses a myriad of conditions, including cerebrovascular disease (stroke), vascular diseases, peripheral artery disease, and other heart-related diseases, including heart failure and coronary heart disease (heart attack) [@gaidai2023global, @gladwell2023traditional].

Numerous epidemiological studies have determined various established risk factors that increase the probability of developing CVD. Some of these factors include increasing age, hypertension, smoking, high cholesterol, obesity, diabetes, gender, and alcohol consumption [@nhs2025cvd]. These factors contribute to CVD through various pathophysiological processes. For example, elevated cholesterol promotes plaque buildup, increasing ASCVD risk, while hypertension stresses arterial walls, causing damage that can lead to plaque formation [@lee2019cholesterol, @aha2025hbp].

To further explore these relationships, the current analysis utilizes an open-source cardiovascular dataset (from Kaggle) containing information from 70,000 adult patients [@Ulianova2018cvd, @Torabi2018cvd]. The dataset includes 13 variables describing each individual’s demographic, behavioral, and clinical characteristics. Each variable is classified into one of three input types: objective, examination, or subjective. Objective inputs refer to factual demographic data, examination inputs derive from medical assessments, and subjective inputs are self-reported by the patient. Examination inputs come from medical assessments, while subjective inputs are self-reported by patients.
...
|Height        | Individual's Height                        |cm                                              |Objective
...
While many CVD studies have explored these risk factors independently, fewer have examined how combinations of these factors interact to influence the development of CVD. An even smaller number have incorporated pulse pressure as a contributing variable. Considering these risks in combination improves identification of individuals at higher CVD risk. Pulse pressure, the difference between systolic and diastolic blood pressure [@homan2018physiology], indicates arterial stiffness and can contribute to CVD [@o1999pulse]. Examining pulse pressure in conjunction with other proven indicators may not only provide a more thorough picture of cardiovascular risk profiles but also allow for more targeted preventative interventions. 

This study aims to understand how various factors, including gender, age, cholesterol level, glucose level, and pulse pressure, influence the likelihood of developing cardiovascular disease amongst the adult population. These variables were selected because they have been consistently associated with cardiovascular risk in epidemiological studies. To achieve this objective, the study employed a structured analytical approach encompassing several stages of data preprocessing and analysis. These steps included data cleaning (e.g., removal of implausible values), derivation of new variables such as pulse pressure, data visualization, and logistic regression modeling.
...
A cohort characteristic table was created using the `tableone` package to summarise age, pulse pressure, gender, cholesterol, and glucose by CVD status, reporting means (SD) for continuous variables and counts (%) for categorical variables.
...
Figure 6.0 shows a scatter plot of systolic versus diastolic blood pressure by CVD status. Blood pressure values overlap between groups, but CVD cases are more concentrated at higher levels. The strong correlation between ap_hi and ap_lo supports using pulse pressure as a derived variable to capture arterial load while reducing redundancy.
...
Figure 7.0 shows the distribution of pulse pressure by CVD status. For individuals without CVD, counts peak at a pulse pressure of 35 before declining, while CVD cases follow a similar pattern but surpass non-CVD counts above 35. This indicates that higher pulse pressure is associated with a greater proportion of individuals with CVD.
...
The Cohort characteristic table shows that individuals with CVD were older and had higher pulse pressure than those without CVD. They also had higher proportions of elevated cholesterol and glucose, while gender distribution was similar between groups. These differences provide context for the subsequent logistic regression analysis.
...
This study aimed to understand how various factors—including gender, age, cholesterol level, glucose level, and pulse pressure—influence the likelihood of developing cardiovascular disease among adults. As expected, our findings suggest that age, elevated pulse pressure, and elevated cholesterol levels were linked to a higher risk of developing CVD. Elevated pulse pressure is likely indicative of arterial stiffness, an established contributor to CVD [@who2025cvd]. Increased cholesterol contributes to atherosclerotic plaque formation, consistent with prior studies [@lee2019cholesterol]. The cohort characteristics table provides context for these results, showing the distribution of key variables such as age, gender, cholesterol, and glucose levels within the study population.

Contrary to expectations, gender proved to be an insignificant predictor within this dataset. As indicated in the cohort characteristics table, the proportion of males to females may partially explain this finding, along with other predictors potentially influencing the relationship between the independent and dependent variables. These findings contradict what many studies have established, with CVD risk generally higher among males [@mikkola2013sex]. Glucose showed mixed results, with Above Normal being associated with higher odds of CVD and Well Above Normal associated with lower odds. These patterns could be influenced by the sample composition (cohort characteristics table) and/or other unaccounted factors, such as genetics, lifestyle choices, and medication use.

These findings emphasize the importance of monitoring pulse pressure and cholesterol as risk factors. They also highlight the need for further research to clarify the role of glucose levels and gender in CVD risk.

Limitations of this study include missing data, the use of cross-sectional data, categorical measurements for cholesterol and glucose, and potential measurement/input errors. Strengths include meticulous data cleaning, comprehensive exploratory data analysis, and multivariable analysis. Future research should consider longitudinal designs, additional clinical features, and lifestyle factors to improve predictive accuracy and validate these findings in larger populations.


# Conclusion

This study identified key predictors for CVD, showing that increasing age, elevated pulse pressure, and higher cholesterol are positively associated with its development. Unexpectedly, increased glucose levels showed an inverse relationship, and gender proved to be an insignificant predictor for CVD risk. These findings, considered alongside the cohort characteristics table, highlight how sample composition may influence observed associations. The multivariable logistic regression model displayed moderate discrimination in distinguishing between patients with and without CVD, supported by the ROC AUC score of 0.7498. Collectively, these results underscore the multifactorial nature of CVD risk, the value of analyzing these factors in conjunction, and the importance of using data to identify key risk factors for prevention. Despite limitations such as data entry errors and categorical variables for glucose and cholesterol, the study provides a clear foundation for further research and more targeted investigations into cardiovascular risk.
